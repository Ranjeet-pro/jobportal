...
public class jobseeker extends JFrame implements ActionListener{
    ...
    jobseeker(){
       getContentPane().setBackground(Color.yellow);
        ...
        image.add(back);
        JLabel search = new JLabel("Search JobSeeker by id:-");
        search.setBounds(50, 160, 450, 30);
        search.setForeground(Color.white);
        search.setFont(new Font("Arial", Font.PLAIN, 25));
        image.add(search);
        String cname[]={"123425","754987","754636"};
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==loginout){
            ...
            try{
               Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from jobseeker where jobseeker_id= '"+coname+"' ");
            table1.setModel(DbUtils.resultSetToTableModel(rs)); 
            }catch(Exception e){
               e.printStackTrace(); 
            }
